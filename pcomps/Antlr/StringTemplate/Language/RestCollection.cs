...
namespace pcomps.Antlr.StringTemplate.Language
{
	...
	internal sealed class RestCollection : ICollection, IEnumerable
	{
		...
		{
            if (o is not RestCollection restCollection) return false;
            if (Count == 0 && restCollection.Count == 0)
            {
                return true;
            }

            if (Count != restCollection.Count) return false;
		...
		public IEnumerator GetEnumerator() => new RestCollectionIterator(this);
		...
		internal sealed record RestCollectionIterator : IEnumerator
		{
